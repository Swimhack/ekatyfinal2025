...
export default function RestaurantDetailClient({ initial }: { initial?: any }) {
  const params = useParams()
  const toast = useToast()
  const [restaurant, setRestaurant] = useState<any>(initial ?? null)
  ...
  useEffect(() => {
    if (params.slug) {
      // The server component hands the listing down so the markup is present on
      // the first pass; only fetch when it wasn't.
      if (!initial) fetchRestaurant(params.slug as string)
      checkFavoriteStatus()
      checkAdminStatus()
    }
  }, [params.slug])
